...
export const useLanguageAtom = (namespace?: string) => {
    ...
    useEffect(() => {
        ...
        document.title = language.language === "cn" ? "主页" : "Home Page";
    }, [language, hydrated])
    ...
    const t = useCallback(
        (key: string): string => {
            if (!namespace) {
                // 无命名空间：直接查顶层字符串
                const direct = resources[language.language][key];
                return typeof direct === "string" ? direct : key;
            }
            // 有命名空间：在命名空间对象中查找
            const nsContent = resources[language.language][namespace];
    ...
    return {
        ...
        setLanguage: (language: "cn" | "en") => setLanguage({language: language}),
        hydrated,
        t
    };
}